...
proto.findOrCreate = function* (table, where, data){
  const item = yield this.get(table, where);
  if(item) return {op:'find',item:item};
  const res = yield this.insert(table,data);
  if(res && res.affectedRows === 1) {
    return {op:'create',item:Object.assign({id:res.insertId},data)};
  }else{
    return null;
  }
};
...
proto.updateOrCreate = function* (table,where,data,option){
  const item = yield this.get(table,where);
  if(item) {
     data.id = item.id;
     const res =  yield this.update(table,data);
     return {op:'update',res:res};
  }else{
    if(option && option.insert){
       data = Object.assign(data,option.insert)
    }
    const res = yield this.insert(table,data);
    return {op:'create',res:res};
  }
};
